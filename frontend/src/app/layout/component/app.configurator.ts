...
export class AppConfigurator implements OnInit {
    ...
    selectedTheme = computed(() => {
        ...
        if (this.layoutService.isFollowingSystemTheme()) {
            return 'system';
        }
        // Otherwise return the manually set theme
        const config = this.layoutService.layoutConfig();
        return config.darkTheme ? 'dark' : 'light';
    });
    ...
    onThemeChange(value: string) {
        if (value === 'system') {
            this.layoutService.setDarkModeToSystem();
        } else {
            // Remove followSystemTheme flag when manually setting theme
            const saved = localStorage.getItem('ednaLayoutConfig');
            if (saved) {
                const config = JSON.parse(saved);
                delete config.followSystemTheme;
                config.darkTheme = value === 'dark';
                localStorage.setItem('ednaLayoutConfig', JSON.stringify(config));
            }

            // Update signal
            this.layoutService.followingSystemTheme.set(false);
